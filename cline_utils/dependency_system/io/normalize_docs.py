...
def load_transparency_registry(project_root: str) -> set[str]:
    registry_path = os.path.join(project_root, "cline_utils", "dependency_system", "core", "transparency_registry.json")
...
def clean_body_content(content: str) -> str:
    ...
    content = re.sub(r"<!--\s*---\s*STATION_HEADER_START\s*---\s*\[AUTO\].*?---\s*STATION_HEADER_END\s*---\s*\[AUTO\]\s*-->", "", content, flags=re.DOTALL)
    ...
    content = re.sub(r"^\s*##\s*(Context|Overview|Details|References)\s*$", "", content, flags=re.MULTILINE | re.IGNORECASE)
...
def normalize_single_file(
    ...
    try:
        ...
        existing_role, existing_layer, crct_key, tracker_ref = parse_station_header(original_content)
        if not crct_key or not tracker_ref:
            print(f"Skipping {rel_path}: Missing valid STATION_HEADER with CRCT_KEY and TRACKER_REF.")
            return False
        ...
        context_match = re.search(r"## Context\s*\n(.*?)(?:\n##|\n---)", original_content, re.DOTALL | re.IGNORECASE)
        if context_match:
        ...
        if prompt_tokens > MAX_INPUT_TOKENS:
            print(f"Skipping {rel_path}: Document total token count ({prompt_tokens}) exceeds maximum input context size ({MAX_INPUT_TOKENS} tokens). Must be processed via cloud LLM.")
            return False
        
        print(f"File size verified suitable ({prompt_tokens} tokens). Running analysis...")
        ...
        if references_body:
            ...
            references_body_clean = re.sub(r"---REFERENCES_(START|END)---", "", references_body).strip()
        ...
        details_body_normalized = re.sub(r"\[([^\]]+)\]\(([^)]+)\)", replace_link, details_body)
...
def main():
    parser = argparse.ArgumentParser(description="Normalize documentation files to structured format using local LLM.")
    parser.add_argument("--dry-run", action="store_true", help="Only process the first 2 files and write to scratch/dry_run/")
    parser.add_argument("--limit", type=int, default=10, help="Max files to process in this run")
    ...
    print(f"Loaded {len(registered_files)} registered files from transparency registry.")
            ...
            for root, _, files in os.walk(doc_dir):
                for file in sorted(files):
                    if file.endswith(".md"):
                        ...
                        # We also check if references are in the details section (which we want to correct).
                        ...
                        if "## Details" in content:
                            details_section = content.split("## Details")[1].split("## References")[0]
                            if any(hdr in details_section for hdr in ["Documentation Links", "Links", "See Also", "Related Files"]):
    ...
    print(f"Found {total_candidates} candidate files that need normalization or placeholder resolution.")
    ...
    if args.dry_run:
        ...
        print(f"\nWill process up to {args.limit} successfully normalized candidate files.")

    # Initialize Local LLM
    model_path = os.path.join(project_root, "models", "Qwen3-4B-Instruct-2507-Q8_0.gguf")
    ...
    for filepath in candidates:
        ...
        if args.dry_run and processed_count >= 2:
            break
        if args.limit and processed_count >= args.limit:
            break
            
        rel_path = os.path.relpath(filepath, project_root)
        print(f"\n[{processed_count+1}/{len(candidates)}] Checking Context Suitability: {rel_path}")
    ...
    processor.close()
    print(f"\nDone! Processed {processed_count} candidate files.")
    return 0
