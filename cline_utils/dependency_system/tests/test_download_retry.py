...
def test_download_success():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            "cline_utils.dependency_system.analysis.embedding_manager.urlopen",
            return_value=response,
        ) as mock_urlopen, patch(
            "cline_utils.dependency_system.analysis.embedding_manager.Request"
        ) as mock_req_class:
            ...
            success = _download_with_retry(
                url, dest_path, "Test Model", max_retries=3, initial_delay=0.01
            )
            ...
            mock_urlopen.assert_called_once_with(mock_req, timeout=30)
...
def test_download_permanent_error():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            ...
            success = _download_with_retry(
                url, dest_path, "Missing Model", max_retries=5, initial_delay=0.01
            )
...
def test_download_transient_retry_success():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            ...
            success = _download_with_retry(
                url, dest_path, "Transient Model", max_retries=3, initial_delay=0.01
            )
...
def test_download_resume_success():
    ...
    response1 = FailingResponse(
        part1_data, status=200, headers={"Content-Length": str(len(total_data))}
    )

    # Second attempt: returns the remaining data with status 206 Partial Content
    response2 = DummyResponse(
        part2_data, status=206, headers={"Content-Length": str(len(part2_data))}
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
        ...
        ) as mock_urlopen, patch("time.sleep") as mock_sleep, patch(
            "cline_utils.dependency_system.analysis.embedding_manager.Request"
        ) as mock_req_class:
            ...
            success = _download_with_retry(
                url, dest_path, "Large Model", max_retries=3, initial_delay=0.01
            )
...
def test_download_resume_ignored_starts_over():
    ...
    response1 = FailingResponse(
        part1_data, status=200, headers={"Content-Length": "1000"}
    )

    # Second attempt: server returns 200 OK (ignores Range header)
    response2 = DummyResponse(
        full_data, status=200, headers={"Content-Length": str(len(full_data))}
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            ...
            success = _download_with_retry(
                url, dest_path, "Non-resumable Model", max_retries=3, initial_delay=0.01
            )
