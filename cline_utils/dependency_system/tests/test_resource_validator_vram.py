...
class TestResourceValidatorVRAM(unittest.TestCase):
    ...
    @patch('cline_utils.dependency_system.utils.resource_validator.TORCH_AVAILABLE', True)
    @patch('cline_utils.dependency_system.utils.resource_validator.torch')
    def test_wait_for_vram_release_success(self, mock_torch):
        # Setup mocks
    ...
    @patch('cline_utils.dependency_system.utils.resource_validator.TORCH_AVAILABLE', True)
    @patch('cline_utils.dependency_system.utils.resource_validator.torch')
    def test_wait_for_vram_release_stall(self, mock_torch):
        # Setup mocks
        mock_torch.cuda.is_available.return_value = True
        ...
        mock_torch.cuda.mem_get_info.side_effect = [
            ...
            (110 * 1024 * 1024, 0), # poll 4 (no growth, stall_count=3 -> stall triggered)
        ]
    ...
    @patch('cline_utils.dependency_system.utils.resource_validator.TORCH_AVAILABLE', True)
    @patch('cline_utils.dependency_system.utils.resource_validator.torch')
    def test_wait_for_vram_release_hard_cap(self, mock_torch):
        # Setup mocks
        mock_torch.cuda.is_available.return_value = True
