...
def test_native_grid_layout_partitions_columns():
    ...
    assert (
        len(x_coords) == 3
    ), f"Expected 3 columns for 10 nodes with max 4/col, got {len(x_coords)}: {x_coords}"
...
def test_native_hex_stagger_offsets_y():
    ...
    config = NativeLayoutConfig(
        max_nodes_per_column=4, enable_hex_stagger=True, hex_stagger_ratio=0.5
    )
    ...
    nodes_sorted = sorted(
        [layout.nodes[f"1A{i+1}"] for i in range(n_children)], key=lambda n: (n.x, n.y)
    )
    ...
    assert (
        col1_nodes[0].y > col0_nodes[0].y
    ), "Column 1 should be staggered (shifted down)"
    ...
    assert col1_nodes[0].y - col0_nodes[0].y == pytest.approx(
        34.0
    ), f"Expected 34px stagger, got {col1_nodes[0].y - col0_nodes[0].y}"
