...
def test_project(temp_test_dir: Path) -> Generator[Path, None, None]:
    ...
    (project_dir / "lib").mkdir(
        exist_ok=True
    )  # Keep for consistency even if not used by FC
...
def test_fc01_get_project_root_cache(
    test_project: Path, clear_cache_fixture, caplog
):  # Added fixture arg
    ...
    assert normalize_path_cached(root2) == normalize_path_cached(
        root1
    )  # Compare normalized
    ...
    assert normalize_path_cached(root3) == normalize_path_cached(
        root1
    )  # Compare normalized
    ...
    assert stats3.get("misses", 0) == stats2.get(
        "misses", 0
...
def test_fc02_is_valid_project_path_cache(
    test_project: Path, clear_cache_fixture, caplog
):  # Added fixture arg
    clear_all_caches()  # Manual clear at start
    ...
    print(
        f"Calling is_valid_project_path_cached for '{another_valid_path_rel}' (1st time)"
    )
...
def test_fc03_calculate_similarity_cache(
    test_project: Path, clear_cache_fixture, monkeypatch, caplog
):  # Added fixture
    ...
    def mock_load_embedding(
        key: str, embedding_path: Path | None = None
    ):  # Adjusted signature
    ...
    try:
        ...
        monkeypatch.setattr(
            embedding_manager, "load_embedding", mock_load_embedding, raising=False
        )
    except AttributeError:
        # If load_embedding isn't directly in embedding_manager, adjust the target path
        print(
            "Warning: Could not directly mock embedding_manager.load_embedding. Cache test relies on mtime checks."
        )
    ...
    sim1 = embedding_manager.calculate_similarity(
        key1,
        key2,
        str(embedding_dir),
        {},
        str(test_project),
        [str(test_project / "src")],
        [str(test_project / "docs")],
    )
    ...
    sim2 = embedding_manager.calculate_similarity(
        key1,
        key2,
        str(embedding_dir),
        {},
    ...
    sim3 = embedding_manager.calculate_similarity(
        key2,
        key1,
        str(embedding_dir),
        {},
    ...
    sim4 = embedding_manager.calculate_similarity(
        key1,
        key2,
        str(embedding_dir),
        {},
    ...
    stats4 = get_cache_stats(cache_name)
    assert stats4["misses"] > stats3.get(
        "misses", 0
    )  # Check increase from previous state
    ...
    sim5 = embedding_manager.calculate_similarity(
        key1,
        key2,
        str(embedding_dir),
        {},
    ...
    sim6 = embedding_manager.calculate_similarity(
        key1,
        key3,
        str(embedding_dir),
        {},
...
def test_fc04_validate_grid_cache(clear_cache_fixture):  # Added fixture
    ...
    assert stats3["hits"] > stats2.get(
        "hits", 0
    ), "Expected cache hit when calling with unsorted keys."
    ...
    assert stats4["misses"] > stats3.get(
        "misses", 0
    ), "Expected cache miss for invalid grid content."
    ...
    assert stats5["misses"] > stats4.get(
        "misses", 0
    ), "Expected cache miss for different keys list."
...
def test_fc05_get_dependencies_from_grid_cache(clear_cache_fixture):  # Added fixture
    ...
    print(
        f"Calling get_dependencies_from_grid(G1, {target_key1}, K1_sorted) (2nd time)"
    )
    ...
    print(
        f"Calling get_dependencies_from_grid(G1, {target_key1}, K1_unsorted) (1st time)"
    )
    keys1_unsorted_alt = make_key_info_list(["1B", "1A2", "1A1"])
    deps3 = dependency_grid.get_dependencies_from_grid(
        grid1, target_key1, keys1_unsorted_alt
    )
    assert deps3 == deps1
    stats3 = get_cache_stats(cache_name)
    ...
    print(
        f"Calling get_dependencies_from_grid(G1, {target_key2}, K1_sorted) (1st time)"
    )
    ...
    print(
        f"Calling get_dependencies_from_grid(G2, {target_key1}, K1_sorted) (2nd time)"
    )
...
def test_fc06_read_tracker_file_structured_cache(
    test_project: Path, clear_cache_fixture
):  # Added fixture
    clear_all_caches()  # Manual clear
...
def test_fc07_config_dependent_caches(
    test_project, clear_cache_fixture, monkeypatch
):  # Added fixture
    clear_all_caches()  # Manual clear
    ...
    test_file_path_excluded = str(
        test_project / "data/log.log"
    )  # Matches *.log exclusion
    ...
    try:
        ...
        pytest.skip(
            "Skipping _is_valid_file test: function not found in embedding_manager"
        )
    ...
    assert stats_valid2["misses"] > stats_valid1.get(
        "misses", 0
    ...
    assert stats_valid3["misses"] > stats_valid2.get(
        "misses", 0
    )  # Increase vs previous
