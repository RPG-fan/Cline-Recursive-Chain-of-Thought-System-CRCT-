...
def test_process_file_identifies_virtualized_maps_as_refreshed(tmp_path: Path):
    ...
    try:
            ...
            "functions": [
                {
                    "name": "source_func",
                    "line": 1,
                    "attribute_accesses": ["target_func"],
                }
            ],
            "classes": [],
        }
        ...
        target_symbol_data = {
            "functions": [{"name": "target_func", "line": 10}],
            "classes": [],
        }

        full_symbol_map = {
            str(source_path): symbol_data,
            str(project_root / "target.py"): target_symbol_data,
        }
        ...
        assert (
            result["maps_updated"] == 1
        ), f"Expected 1 refreshed map, got {result['maps_updated']}"
        assert (
            result["maps_added"] == 0
        ), f"Expected 0 new maps, got {result['maps_added']}"
