...
def merge_runtime_and_ast(
    runtime_symbols: Dict[str, Dict[str, Any]], ast_analysis: Dict[str, Dict[str, Any]]
    ...
    for file_path, runtime_data in runtime_symbols.items():
        ...
        for rt_class in runtime_classes:
            ...
            matching_ast = next(
                (c for c in ast_classes if c.get("name") == rt_class["name"]), None
            )
        ...
        for rt_func in runtime_functions:
            matching_ast = next(
                (f for f in ast_functions if f.get("name") == rt_func["name"]), None
            )
...
def save_merged_symbol_map(
    merged_map: Dict[str, Dict[str, Any]], output_path: str, backup_old: bool = True
) -> None:
    """
...
def validate_merged_output(
    ...
    """
    issues = {"missing_runtime_data": [], "missing_ast_data": [], "info": []}
