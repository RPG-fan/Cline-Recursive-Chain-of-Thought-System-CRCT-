---
description: Runs the Cline Recursive Chain-of-Thought (CRCT) framework for phase-driven project management with persistent state and dependency tracking. Use when a task should follow the CRCT workflow (Set-up/Maintenance, Strategy, Execution, Cleanup/Consolidation), when dependency analysis via dependency_processor is required, or when maintaining the project's HDTA docs and trackers.
mode: all
---
...
*`python -m cline_utils.dependency_system.dependency_processor` is a CLI operation and should be run via the `bash` tool.*
...
When modifying files, selecting the most appropriate and token-efficient tool is crucial for system performance and operational cost-effectiveness. In opencode, file changes use two tools: `edit` (targeted changes) and `write` (full-file create/overwrite). Adhere to the following prioritization and guidelines:

1. **`edit` for insertion**:
    * **Use Case**: Your **primary choice** when the task involves **only adding new content** to a file without altering or deleting any existing lines. Provide the surrounding context (the `oldString` anchor) so the tool can locate the exact insertion point.
    * **Examples**:
    ...
    * **Rationale**: `edit` is highly efficient because it transmits only the content to be inserted and the unique anchor that locates it, minimizing token usage compared to rewriting larger portions of the file.
    *Be very careful to match the **indentation** of the content you are inserting to*

2. **`edit` for search-and-replace / multi-part changes**:
    * **Use Case**: Utilize `edit` when you need to **edit, modify, or change existing content within localized areas** of a file.
        * **Simple substitutions**: Use `edit` with an `oldString`/`newString` pair for straightforward find-and-replace operations. Suitable for simple text substitutions or repeated pattern-based changes. Best when changes are repetitive or described by a single search/replace pair.
        * **Complex, multi-part changes**: Several targeted `edit` operations (or one `edit` whose `oldString` spans multiple related lines) are more suitable for refactorings that change a function signature and its internal logic, or update multiple distinct but related lines.
    * **Examples**:
        * Refactoring a variable or function name throughout a specific scope or an entire file (repeated `edit` operations).
        * Modifying parameters of an existing function and updating its calls within the same file (one `edit` covering multiple related changes, or several targeted `edit` operations).
        * Correcting typos, updating specific values in configuration files, or changing comments.
    * **Rationale**: `edit` is significantly more token-efficient than `write` for modifications because it operates on specific sections or patterns rather than requiring the transmission and processing of the entire file content.

3. **`write`**:
    * **Use Case**: This tool should be employed **only as a last resort** when `edit` is clearly inadequate or overly cumbersome. Typical scenarios include:
        * Creating a brand new file from scratch.
        * When the required changes are so extensive, pervasive, and non-pattern-based throughout the file that using `edit` would be more complex or less clear than respecifying the entire file content.
    ...
    * **Rationale**: `write` consumes the most context/tokens because it necessitates sending the *complete* intended content of the file. Its use should always be carefully justified by the inability of more efficient tools to perform the task effectively.

**Critical Scrutiny of Tool Selection for File Modifications:**
Before proposing the use of any file modification tool, and **especially before resorting to `write`**, you MUST critically evaluate if a more targeted and token-efficient `edit` could achieve the same result. If `write` is chosen, explicitly state your reasoning, justifying why `edit` is not suitable for the specific modification task. This diligence is paramount for maintaining system performance, minimizing operational token costs, and ensuring precise, auditable changes. Any suggestion to use a less optimal tool without justification will be considered a deviation from standard operating procedure.
...
**If you feel like the task is finished, *first* perform the MUP as detailed in section `VI. Mandatory Update Protocol (MUP) - Core File Updates`** before returning your final result.
